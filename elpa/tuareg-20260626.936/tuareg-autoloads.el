...
In addition to any hooks its parent mode `tuareg-mode' might have run, this mode
runs the hook `tuareg-interface-mode-hook', as the final or penultimate step
...
runs the hook `tuareg-menhir-mode-hook', as the final or penultimate step during
initialization.
